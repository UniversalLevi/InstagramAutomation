"""
Media Queue System: Manage posts in queue, integrate with database.
"""
...
MEDIA_PHOTOS = MEDIA_QUEUE / "photos"
MEDIA_VIDEOS = MEDIA_QUEUE / "videos"
MEDIA_REELS = MEDIA_QUEUE / "reels"
MEDIA_CAROUSELS = MEDIA_QUEUE / "carousels"


def _ensure_media_directories():
    """Create media directories if they don't exist."""
    for dir_path in [MEDIA_QUEUE, MEDIA_POSTED, MEDIA_FAILED, MEDIA_PHOTOS, MEDIA_VIDEOS, MEDIA_REELS, MEDIA_CAROUSELS]:
        dir_path.mkdir(parents=True, exist_ok=True)


class MediaQueue:
    """Manages post queue using SQLite database."""
    ...
    def get_next_post(self, account_id: Optional[str] = None, db_path: Optional[Path] = None) -> Optional[PostItem]:
        """Get next post ready to be posted (pending or scheduled time reached)."""
        db_path = db_path or self.db_path
    ...
    def update_status(
        ...
        with db_module.cursor(db_path) as cur:
            if status == PostStatus.POSTED:
                ...
                # PENDING / SCHEDULED / POSTING: clear error_message when resetting to pending
                cur.execute(
    ...
    def mark_posted(self, post_id: int, success: bool = True, error_message: Optional[str] = None, db_path: Optional[Path] = None):
        """Mark post as posted and move files."""
        db_path = db_path or self.db_path
        
        # Get post to find file paths
    ...
    def list_queue(
        ...
        """List posts in queue with optional filters."""
        db_path = db_path or self.db_path
        conditions = []
        params = []
    ...
    def _row_to_post_item(self, row) -> PostItem:
        """Convert database row to PostItem."""
